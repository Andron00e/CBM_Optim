...
def make_loss_plot(ax, hist, loss_name, eps=0.01, alpha=0.5, make_train=True, make_val=True, starting_epoch=0,
                   group_unique=False):
    if len(hist) < 7:
        colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    else:
...
def make_metrics_plot(ax, hist, eps=0.01, alpha=0.5, make_train=True, make_val=True, starting_epoch=0,
                      metric_name="acc_top_1", title="top-1 accuracy"):
    if len(hist) < 7:
        colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    else:
        cmap = plt.get_cmap('hsv')
...
def make_accuracy_plot(ax, hist, eps=0.01, alpha=0.5, top_k=1, make_train=True, make_val=True, starting_epoch=0):
    return make_metrics_plot(ax, hist, eps, alpha, make_train, make_val, starting_epoch,
                             metric_name="acc_top_{}".format(top_k), title="top-{} accuracy".format(top_k))


def make_plot(
        ax, hist, y_name, x_name,
        title="Training", y_label="loss", x_label="iteration",
...
class BottleneckTrainer:
    def __init__(self, nets, opts, num_epochs, criterion_cbl, criterion_head, train_loader, val_loader,
                 test_loader, task_name, opt_names, bs_muls, lr_decay=1):
        self.nets = nets
        self.opts = opts
    ...
    def initialize_history(self):
        hist = []
        for (net, optimizer, opt_name, bs_mul) in zip(self.nets, self.opts, self.opt_names, self.bs_muls):
            hist.append({
    ...
    def train(self, batch_mul_step_count=400, calc_norm_diffs=False):
        ...
        for epoch in range(self.num_epochs):
                ...
                for i, data in enumerate(self.train_loader, 0):
                    ...
                    if calc_norm_diffs and ((total_steps - prev_grad_norms_eval_step) > norm_diffs_step_count or
                        ...
                        norm_diffs = compute_norm_diffs(
                            net, optimizer_head, criterion_head,
                            self.train_loader, self.train_loader, full_grad=None, repeats=5
                        )
                    ...
                    if (total_steps - prev_val_eval_step) > val_step_count and net_hist["batch_end"]:
                        ...
                        if total_steps % 10 == 0:
                            ...
                            grouped_hist = group_uniques_full(net_hist, ["train_loss", "train_cbl_loss",
                                                                     "val_loss", "val_cbl_loss",
                                                                     "train_acc_top_1", "train_acc_top_5"
                                                                     "val_acc_top_1", "val_acc_top_5"
                                                                     ])

                            fig = plt.figure(figsize=(15, 8 + 2 * ((len(grouped_hist) + 2) // 3)), dpi=300)
                            ...
                            ax1 = make_loss_plot(
                                ax1, grouped_hist, loss_name="CE loss",
                                eps=0.01, make_val=False, alpha=0.9)
                            ...
                            if calc_norm_diffs:
                                draw_norm_hists_for_different_models(fig, gs[4:, :], grouped_hist, bins_n=100,
                                                                     draw_normal=True)
            ...
            }, os.path.join(loading_path, f'{net_hist["val_acc_top_1"]}_checkpoint_{epoch}_epoch.pth'))
    ...
    def test(self):
        ...
        return {"Top 1 accuracy": net_hist["test_acc_top_1"],
